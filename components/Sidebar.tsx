...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onModeSelect, onClose, credits, currentLang }) => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div 
          className="fixed inset-0 bg-black/95 z-40 md:hidden backdrop-blur-md"
          onClick={onClose}
        />
      )}
      ...
      <div className={`
        ...
        flex flex-col shadow-2xl font-sans bg-[#020409]/90
      `}>
        <div className="p-8 border-b border-skillfi-neon/10 flex justify-between items-center bg-transparent">
          <div>
              <h2 className="font-bold font-display text-white text-2xl tracking-[0.2em] text-shadow-gold">{t('menu', currentLang)}</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 scrollbar-hide">
          {navItems.map((item) => (
              <button
                ...
                className={`w-full flex items-center justify-between p-4 rounded-none border-b border-white/5 transition-all duration-300 group relative overflow-hidden ${
                    item.id === 'logout' 
                    ? 'hover:bg-red-900/10 text-red-400 mt-8 border-none' 
                    : 'hover:bg-white/5 text-gray-400 hover:text-skillfi-neon'
                }`}
              >
                ...
                <div className="flex items-center gap-4 relative z-10">
                    ...
                    <div className="flex flex-col items-start text-left">
                      <span className={`text-sm font-bold tracking-[0.15em] uppercase font-display ${item.id === 'logout' ? 'text-red-400' : 'text-gray-200 group-hover:text-skillfi-neon'}`}>
                          {item.label}
                      </span>
                    </div>
                </div>
